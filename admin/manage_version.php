...
?>
		...
		<center><h4>จัดการเวอร์ชั่น</h4></center>
...
<?php if(isset($_GET['type'])AND $_GET['type']!=0){?>
		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		        <center>
		        	...
		        	<?php $rsVersion = $sql->select('*','version',"type_id='".$_GET['type']."'"); ?>
			        	<form action="" method="POST"> 
			        		คัดลอกตามเวอร์ชั่นที่ <select name="version">
			        			<option value="empty">เวอร์ชั่นเปล่า</option>
				        		<?php while($rwVersion = $rsVersion->fetch_assoc()){ ?>
				        			<option value="<?php echo $rwVersion['version']; ?>"><?php echo $rwVersion['version']; ?></option>
				        		<?php } ?>
			        		</select>
			     <input type="hidden" name="type_id" value="<?php echo $_GET['type']; ?>">
...
<table class="table table"> 
  ...
  <thead>
    <tr>
      ...
      <th style="border-top-width: 0;" scope="col">เวอร์ชั่น</th>
  ...
  <tbody>
    ...
    <tr><?php
		...
		<tr>
			...
			<th scope="row"><center><div style="text-align: center; padding:4px;background-color: <?php echo $color; ?>;width: 38px;border-radius: 3px;color:white"><?php echo $status; ?></div></th> 
			<th scope="row">
...
<?php
	...
	if(isset($_POST['btn_add'])){
		...
		if($_POST['version']=="empty"){
			$sql->insert("step","step,type_id,version","'0','$type_id','$version'");	
		}else{
			$old_version = $_POST['version'];
			$resultStep =  $sql->select("*","step","version='$old_version' AND type_id='$type_id'");
			while ($row = $resultStep->fetch_assoc()) {
				$pos_id = $row['position_id'];
				$step   = $row['step'];
				$sql->insert("step","step,type_id,version,position_id","'$step','$type_id','$version','$pos_id'");	
			}
		}
				$alert->status("10");
				$alert->link("0","manage_version.php");
