...
?>
		...
		<center><h4> การดำเนินการที่จบแล้ว </h4></center>
 <?php 
	$perpage = 7;
	$start = 0;
	...
	if(isset($_GET['txt_search'])AND($_GET['txt_search'])!=''){
		$result=$sql->select("*","activity left join activity_type on activity.type = activity_type.activity_type_id","finish!='' AND (topic like '%".$_GET['txt_search']."%' OR activity_type.type like '%".$_GET['txt_search']."%') $acttype ORDER BY activity_id DESC LIMIT $start , $perpage ");
		$result_page=$sql->select("*","activity left join activity_type on activity.type = activity_type.activity_type_id","finish!='' $acttype AND topic like '%".$_GET['txt_search']."%' OR activity_type.type like '%".$_GET['txt_search']."%' ");
	}else{
		$result=$sql->select("*","activity left join activity_type on activity.type = activity_type.activity_type_id","finish!='' $acttype ORDER BY activity_id DESC LIMIT $start , $perpage");
		...
		<?php 
			if($showpage>1){
				for ($i=1; $i <= $showpage ; $i++) { 
					if(isset($_GET['txt_search'])AND($_GET['txt_search'])!=''){
						echo "<a href='finish_activity.php?page=$i&txt_search=".$_GET['txt_search']."' style='margin-right:20px'>$i</a>";
					}else{
						echo "<a href='finish_activity.php?page=$i' style='margin-right:20px'>$i</a>";
					}
				} 
			}
		?>
